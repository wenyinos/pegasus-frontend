...
{
    return paths::writableCacheDir() + QStringLiteral("/gamecache.json");
}
} // namespace
...
void ProviderManager::run()
{
    ...
    m_future = QtConcurrent::run([this]{
        ...
        providers::game_cache::GameCache cache(default_cache_path());

        // Try loading from cache (permanent until manual refresh)
        if (cache.exists()) {
            ...
            Log::warning(LOGMSG("GameCache"), LOGMSG("Cache corrupted, falling back to full scan"));
        }
        ...
        cache.save(m_found_collections, m_found_games);
